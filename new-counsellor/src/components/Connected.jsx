...
getAllCounsellor()
  ...
  let tempConnected = latestData

  tempConnected[dataIndex].counsellorNo = allCounsellor[counsellorIndex].counselorNo;
  tempConnected[dataIndex].counsellorName = allCounsellor[counsellorIndex].Name

  console.log("tempConnected =",tempConnected)

  setLatestData(tempConnected)
...
return (`${year}-${month}-${day}`)
 ...
 return(
    <div>
            <h3>Connected - {latestData.length}</h3>
  ...
  }
  {
    ...
    <td>
      ...
      <option disabled selected value="selected">--Select Counsellor--</option>
        {allCounsellor.map((data, index) => {
          {console.log("counsellor name =",element.counsellorName,data.Name)}
          return (
