...
getAllCounsellor()
  ...
  const getLeadFilter = async () => {
    ...
    try {
      ...
      let totalLead = await fetch('https://counsellorlead-2.onrender.com/getcounselorVisitFilter', {
        method: 'GET',
        headers: {
...
return (`${year}-${month}-${day}`)
 ...
 return(
    <div>
            <h3>All Visit - {latestData.length}</h3>
    ...
    <td>
      <select
        ...
        defaultValue={(allCounsellor.find(item => item.counselorNo === element.counselorNo)).Name} // Assuming you need to pass the selected value
         // Assuming you need to pass the selected value
      >  
        {allCounsellor.map((data, index) => {
          return (
