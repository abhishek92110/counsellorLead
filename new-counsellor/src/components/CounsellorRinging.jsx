...
const addDemo = (element, index, value) => {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      Swal.fire({
        ...
        text: `Demo successfully rescheduled with trainer ID: ${trainer}`
      });
    }
  });
};
  ...
  const setAssignedCounsellor = (index)=>
    ...
    let tempCounsellorRinging = latestData

    tempCounsellorRinging[dataIndex].counsellorNo = allCounsellor[counsellorIndex].counselorNo;
    tempCounsellorRinging[dataIndex].counsellorName = allCounsellor[counsellorIndex].Name

    setLatestData(tempCounsellorRinging)
...
const getLead = async()=>{
  ...
  try
  {
    let totalLead = await fetch('https://counsellorlead-1.onrender.com/getCounsellorRingingConnectedRegistered', {
      method: 'GET',
      headers: {
...
export default CounsellorRinging;
